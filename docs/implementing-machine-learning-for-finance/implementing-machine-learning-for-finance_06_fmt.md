...
本章介绍参数化方法，也称为线性回归法。我们使用这种方法来确定自变量（连续或分类变量）与因变量（始终为连续变量）之间关系的性质。自变量可以是连续变量或分类变量，而因变量则必然是连续变量。该方法研究自变量的变化如何影响因变量的变化。用于估计截距和斜率的最传统模型是最小二乘模型。公式如公式 6-1 所示。
