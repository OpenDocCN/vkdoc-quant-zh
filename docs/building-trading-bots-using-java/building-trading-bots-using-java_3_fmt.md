...
public class FadeTheMoveStrategy < T > {
 ...
 private final Map < TradeableInstrument < T > ,
 Cache < DateTime,
 MarketDataPayLoad < T >>> instrumentRecentPricesCache = Maps
 .newHashMap();
 ...
 public void init() {
  for (TradeableInstrument < T > instrument: instruments) {
   Cache < DateTime, MarketDataPayLoad < T >> recentPricesCache =
 ...
 public void handleMarketDataEvent(MarketDataPayLoad < T > marketDataPayLoad) {
  if (instrumentRecentPricesCache.containsKey(
    marketDataPayLoad.getInstrument())) {
   instrumentRecentPricesCache.get(marketDataPayLoad.getInstrument())
...
public class FadeTheMoveStrategyDemo {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println(String.format(
            "策略指示在 %s 上对工具 %s 进行 %s 操作，限价 %2.5f，止盈 %2.5f ",
            decision.getSignal().name(), decision.getInstrument().getInstrument(),
...
```java
/**
 * 提供来自交易平台的流式心跳的服务。平台最终提供的服务可能根本不是流式的，
 * 而是某种形式的定期回调，用以表明连接处于活动状态。心跳丢失可能表明无法接收
 * 来自交易平台的任何交易/订单事件和/或市场数据。因此，对应用程序的任何监控
 * 都可能涉及直接与此服务交互，以触发警报/通知。
 */
public interface HeartBeatStreamingService {

    /**
     * 启动服务以接收来自交易平台的心跳。理想情况下，实现应确保不会为同一类型的
     * 服务创建多个心跳连接/处理器。根据交易平台的不同，可能所有服务共享一个心跳，
     * 也可能为市场数据、交易/订单事件等服务各自配备专属心跳。
     */
    void startHeartBeatStreaming();

    /**
     * 停止服务以停止接收心跳。实现必须妥善释放所有资源/连接，以免造成任何资源泄漏。
     */
    void stopHeartBeatStreaming();

    /**
     * @return 心跳源标识，用于标识该服务为之提供心跳的源。这对于跟踪所有正在发送心跳
     *         的源非常有用，可以定期单独监控每个源。在某些平台上，可能所有服务共享
     *         一个单一心跳服务，此时该标识可能不那么有用。
     */
...
```json
{"heartbeat":{"time":"1443968021000000"}}
```
...
```java
public abstract class OandaStreamingService
    implements HeartBeatStreamingService {
    protected OandaStreamingService(String accessToken,
...
private final Map<String, HeartBeatStreamingService> heartBeatProducerMap =
    Maps.newHashMap();
private final Map<String, HeartBeatPayLoad<T>> payLoadMap =
    Maps.newConcurrentMap();
volatile boolean serviceUp = true;
protected final Collection<HeartBeatStreamingService>
    heartBeatStreamingServices;
protected final long initWait = 2000L;
long warmUpTime = MAX_HEARTBEAT_DELAY;

public AbstractHeartBeatService(Collection<HeartBeatStreamingService>
    heartBeatStreamingServices) {
    this.heartBeatStreamingServices = heartBeatStreamingServices;
    for (HeartBeatStreamingService heartBeatStreamingService:
        heartBeatStreamingServices) {
        this.heartBeatProducerMap.put(
            heartBeatStreamingService.getHeartBeatSourceId(),
...
final Thread heartBeatsObserverThread = new Thread(new Runnable() {
    ...
    private void sleep() {
        try {
            Thread.sleep(warmUpTime); /*等待流正常启动*/
    ...
    public void run() {
        while (serviceUp) {
            ...
            for (Map.Entry<String, HeartBeatStreamingService> entry:
                ...
                long startWait = initWait; // 从 2 秒开始
                while (serviceUp && !isAlive(payLoadMap.get(entry.getKey()))) {
                    ...
                    // 等待时间加倍，但不超过 MAX_HEARTBEAT_DELAY
...
public class DefaultHeartBeatService
    ...
    protected boolean isAlive(HeartBeatPayLoad<DateTime> payLoad) {
        return payLoad != null && (DateTime.now().getMillis() -
            payLoad.getHeartBeatPayLoad().getMillis()) < MAX_HEARTBEAT_DELAY;
    }
...
public class DefaultHeartBeatServiceDemo {
    ...
    private static void usageAndValidation(String[] args) {
        if (args.length != 3) {
            LOG.error("用法: DefaultHeartBeatServiceDemo <url> <accountid> <accesstoken>");
            System.exit(1);
        } else {
    ...
    public static void main(String[] args) throws Exception {
        ...
        MarketEventCallback<String> mktEventCallback =
            new MarketEventHandlerImpl<String>(eventBus);
        HeartBeatCallback<DateTime> heartBeatCallback =
            new HeartBeatCallbackImpl<DateTime>(eventBus);

        OandaMarketDataStreamingService mktDataStreaminService =
            new OandaMarketDataStreamingService(url, accessToken,
...
```xml
<bean id="accountDataProvider" class=
  "com.precioustech.fxtrading.oanda.restapi.account.OandaAccountDataProviderService">
...
```xml
<bean id="providerHelper"
    class="com.precioustech.fxtrading.oanda.restapi.helper.OandaProviderHelper"/>
```
...
```xml
<bean id="currentPriceInfoProvider"
  class="com.precioustech.fxtrading.oanda.restapi.marketdata.OandaCurrentPriceInfoProvider">
    <constructor-arg index="0" value="${oanda.url}"/>
...
```xml
<bean id="historicMarketDataProvider"
  class="com.precioustech.fxtrading.oanda.restapi.marketdata.historic.OandaHistoricMarketDataProvider">
    <constructor-arg index="0" value="${oanda.url}"/>
    <constructor-arg index="1" value="${oanda.accessToken}"/>
</bean>
```
...
```xml
<bean id="orderManagementProvider"
  class="com.precioustech.fxtrading.oanda.restapi.order.OandaOrderManagementProvider">
    <constructor-arg index="0" value="${oanda.url}"/>
    <constructor-arg index="1" value="${oanda.accessToken}"/>
...
```xml
<bean id="tradeManagementProvider"
  class="com.precioustech.fxtrading.oanda.restapi.trade.OandaTradeManagementProvider">
    <constructor-arg index="0" value="${oanda.url}"/>
    <constructor-arg index="1" value="${oanda.accessToken}"/>
</bean>
```
...
```xml
<bean id="positionManagementProvider"
  class="com.precioustech.fxtrading.oanda.restapi.position.OandaPositionManagementProvider">
    <constructor-arg index="0" value="${oanda.url}"/>
    <constructor-arg index="1" value="${oanda.accessToken}"/>
</bean>
```
...
```xml
<bean id="eventsStreamingService"
  class="com.precioustech.fxtrading.oanda.restapi.streaming.events.OandaEventsStreamingService">
...
```xml
<bean id="marketDataStreamingService"
  class="com.precioustech.fxtrading.oanda.restapi.streaming.marketdata.OandaMarketDataStreamingService">
    <constructor-arg index="0" value="${oanda.streaming.url}"/>
    <constructor-arg index="1" value="${oanda.accessToken}"/>
...
<bean id="tradeableInstrumentList" class="java.util.ArrayList">
    <constructor-arg index="0">
        <list>
            <bean class="com.precioustech.fxtrading.instrument.TradeableInstrument">
                <constructor-arg index="0" value="USD_CAD"/>
            </bean>
            <bean class="com.precioustech.fxtrading.instrument.TradeableInstrument">
                <constructor-arg index="0" value="GBP_USD"/>
            </bean>
            <bean class="com.precioustech.fxtrading.instrument.TradeableInstrument">
                <constructor-arg index="0" value="AUD_JPY"/>
            </bean>
            <bean class="com.precioustech.fxtrading.instrument.TradeableInstrument">
                <constructor-arg index="0" value="EUR_NZD"/>
            </bean>
            <bean class="com.precioustech.fxtrading.instrument.TradeableInstrument">
                <constructor-arg index="0" value="GBP_CHF"/>
            </bean>
            <bean class="com.precioustech.fxtrading.instrument.TradeableInstrument">
                <constructor-arg index="0" value="EUR_JPY"/>
            </bean>
        </list>
    </constructor-arg>
</bean>
...
<task:scheduled-tasks scheduler="taskScheduler">
    <task:scheduled ref="fadeTheMoveStrategy" method="analysePrices"
          fixed-delay="60000"/>
    <task:scheduled ref="copyTwitterStrategy" method="harvestAndTrade"
          fixed-delay="300000"/>
...
public void accountIdTest() throws Exception {
 ...
 OandaAccountDataProviderService spy =
  createSpyAndCommonStuff("src/test/resources/account123456.txt", service);
...
public void createOrderTest() throws Exception {
    OandaOrderManagementProvider service =
        new OandaOrderManagementProvider(OandaTestConstants.url,
            OandaTestConstants.accessToken, null);
    TradeableInstrument<String> eurjpy = new TradeableInstrument<String>("EUR_JPY");

    OandaOrderManagementProvider spy =
        doMockStuff("src/test/resources/newOrder.txt", service);
    ...
    Long orderId =
        spy.placeOrder(orderMarket, OandaTestConstants.accountId);
    assertNotNull(orderId);
    verify(spy, times(1))
        .createPostCommand(orderMarket, OandaTestConstants.accountId);
    ...
    TradeableInstrument<String> eurusd =
        new TradeableInstrument<String>("EUR_USD");
    ...
    assertNotNull(orderId);
    verify(spy, times(1))
        .createPostCommand(orderLimit, OandaTestConstants.accountId);
...
private OandaOrderManagementProvider doMockStuff(String fname,
    OandaOrderManagementProvider service)
    throws Exception {
