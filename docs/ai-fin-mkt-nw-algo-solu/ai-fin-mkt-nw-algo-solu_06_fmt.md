...
代理根据三个主要决策参数行事；贪婪度、真实视野和趋势视野。此外，它们拥有一定数量的资源和总净值。关于这 5 个特征的更多细节在表 6.1 中。
...
**if** `W(N_j^i) > W(A_i)` **then**

**if** `G(N_j^i) >= G(A_i)` **then**

`G(A_i) = G(A_i) * (1 + deg(N_j^i) / V)`
...
**if** `T(N_j^i) >= T(A_i)` **then**

`T(A_i) = T(A_i) * (1 + deg(N_j^i) / V)`
...
**if** `D(N_j^i) >= D(A_i)` **then**

`D(A_i) = D(A_i) * (1 + deg(N_j^i) / V)`
