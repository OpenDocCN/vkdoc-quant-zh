...
表 5-2 显示数据中存在缺失值。清单 5-6 用平均值替换了这些缺失值（数据中的每个变量都执行此操作）。
...
清单 5-19 获取了南非出生时预期寿命的年度变化。接着，它确定了序列的方向。最后，它通过应用 `get_dummies()` 方法*对*响应变量进行了二值化处理。二值化一个方法意味着将其转换为分类变量（即“否”或“是”，其中将“否”编码为 `0`，“是”编码为 `1`）。
...
为了研究逻辑回归模型的性能，清单 5-24 创建了一个*混淆*矩阵（见表 5-8）。要评估一个分类模型，你需要应用与线性回归模型不同的评估指标。在分类问题中，你分析模型的准确率和精确度（见表 5-10）。初始步骤涉及构建混淆矩阵（见表 5-8）。
