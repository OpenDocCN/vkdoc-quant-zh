...
与前几章介绍的模型（`k-means` 除外）不同，该模型不需要您分割数据。此外，它还允许重复检查数据，从而清晰呈现变化。而且，它不包含复杂的评估指标；您可以做的只是最大回撤（极端负峰值），包括模拟结果的均值和标准差。在金融领域，使用模拟方法在无需亲身暴露风险的情况下理解风险非常普遍（在此处了解更多^(⁴)）。
...
**表 9-4** 英国通胀率与消费价格：蒙特卡洛模拟描述性统计摘要
