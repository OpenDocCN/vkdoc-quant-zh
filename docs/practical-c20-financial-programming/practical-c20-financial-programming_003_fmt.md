...
在这个公式中，`N` 是观测值的数量，`x[i]` 是第一个时间序列的第 i 个观测值，`y[i]` 是第二个时间序列的第 i 个观测值，`x̄` 和 `ȳ` 是两个价格序列的均值（平均值），`s[x]` 是 x 值的标准差，`s[y]` 是 y 值的标准差。
...
以下是使用`FundamentalsCalculator`类可以获取的项目列表及其定义方式：
